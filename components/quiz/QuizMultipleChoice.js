...
export default function QuizMultipleChoice({ fruit, allFruits, categoryId, onBack }) {
  const [currentQuestion, setCurrentQuestion] = useState(() => generateQuestion(fruit, categoryId, allFruits));
  ...
  const handleNext = () => {
    const nextQ = generateQuestion(fruit, categoryId, allFruits);
          ...
          <div className="mt-8 animate-float text-center">
            {isCorrect ? (
              <div className="bg-green-500 text-white p-6 rounded-3xl border-b-8 border-green-700">
                ...
                <button onClick={handleNext} className="bg-white text-green-600 px-8 py-3 rounded-xl font-black hover:scale-105 transition-transform">Next ➡</button>
              </div>
            ) : (
              <div className="bg-rose-500 text-white p-6 rounded-3xl border-b-8 border-rose-700">
                ...
                <p className="font-bold mb-4">Correct: {currentQuestion.correctAnswer}</p>
                <button onClick={handleNext} className="bg-white text-rose-600 px-8 py-3 rounded-xl font-black hover:scale-105 transition-transform">Try Again ↺</button>
              </div>
            )}
          </div>
        )}
