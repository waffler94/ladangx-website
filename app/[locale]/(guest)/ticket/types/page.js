...
export default function page() {
    ...
    const {
        ...
        refresh,
    } = useGetCart({ visit_date: searchParams.get("date") });

    const isNewCart =
        cartData?.res_status != 200 || cartData.data.cart.length == 0;
    ...
    const submitHandler = async (e) => {
        ...
        if (isNewCart) {
            ...
            res = await updateCart({
                ...
                details: [
                    ...Object.entries(ticketQuantities).map(([ticket_id, quantity]) => ({
                        ticket_type_id: ticket_id,
                        quantity,
                    })),
                ],
    ...
    return (
        <div className=" min-h-screen  relative pb-[120px] pt-safe">
            <Image
                src="/images/bg16-ticket.png"
                alt="Background"
                fill
                className="object- absolute object-[60%_0%] "
            />
            <div className="relative z-10">
                ...
                <div className="mt-[31px] w-full px-[20px] pb-[120px]">
                    {ticketListLoading ||
                        (isCartLoading && (
                            <div className="space-y-6">
                                <div className="bg-white rounded-lg p-4 h-[200px] shadow-md animate-pulse"></div>
                    ...
                    {!ticketListLoading &&
                        ticketList?.data &&
                        Object.keys(ticketList.data).map((categoryName, categoryIndex) => (
                            <div key={categoryName} className="mb-6">
                                <div className="bg-white rounded-lg p-4 shadow-md">
                                    ...
                                    {ticketList.data[categoryName].map((ticket, index) => {
                                        ...
                                        const itemTotal = calculatePromoPrice(
                                            quantity,
                                            ticket.price,
                                            ticket.promos,
                                        );
                                        ...
                                        return (
                                            <div
                                                key={ticket.id}
                                                className={` flex flex-col ${index < ticketList.data[categoryName].length - 1 ? "mb-4" : ""}`}
                                            >
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <button
                                                            ...
                                                            type="button"
                                                        >
                                                            <i className="icon-deduct_thick text-[20px] text-white" />
                                                        </button>
                                                        ...
                                                        <button
                                                            ...
                                                            type="button"
                                                        >
                                                            <i className="icon-add_thick text-[20px] text-white" />
                                                        </button>
                                                    </div>
                                                </div>
                                                <div className="bg-[#F6F3E1] flex flex-row text-[#60756E] p-3 rounded-[8px] mt-4 gap-2 text-sm">
                                                    <span className="mt-[1px]">•</span>
                                                    <span>
                                                        {t.rich(`ticket_info.${categoryIndex}.${index}`, {
                                                            b: (chunks) => (
                                                                <span className="font-semibold text-[#446A2A]">{chunks}</span>
                                                            ),
                                                        })}
                                                    </span>
                                                </div>
                ...
                <div className="fixed bottom-0 bg-white py-[20px] px-[25px] rounded-t-[20px] w-full drop-shadow-md">
                    {!(
                        ticketList?.data &&
                        Object.values(ticketList.data)
                            .flat()
                            .filter((ticket) => ticketQuantities[ticket.id] > 0).length === 0
                    ) && (
                            <div className="text-[13px] text-[#60756E] mb-2">
                                <h1>{t("your_selection")}</h1>
                            </div>
                        )}
