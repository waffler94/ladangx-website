...
export default function page() {
    ...
    const { data, isLoading, isError } = useGetVouchers({ per_page: 10, page: 1 });

    const handleClick = (id) => {
        openModal(modalList.voucherDetail.key, { data: data.data.find(v => v.id === id) });
    }
            ...
            <div className="flex flex-col w-full gap-[16px]">
                {vouchers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center text-gray-400">
                        ...
                        <p className="text-lg font-semibold">{t("no_available_vouchers_title")}</p>
                        <p className="text-sm">{t("no_available_vouchers_desc")}</p>
                    </div>
                ) : (
                    vouchers.map((voucher) => (
                        <VoucherCard
                            ...
                            icon="icon-voucher"
                            points={voucher.points_required === 0 ? 'FREE' : `${voucher.points_required.toLocaleString()} pts`}
