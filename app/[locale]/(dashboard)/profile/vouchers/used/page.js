...
export default function page() {
    ...
    const { data, isLoading, isError } = useGetVouchers({ per_page: 10, page: 1, user_voucher: 1, used_only: 1 });
            ...
            <div className="flex flex-col w-full gap-[16px]">
                {vouchers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center text-gray-400">
                        ...
                        <p className="text-lg font-semibold">{t("no_used_vouchers_title")}</p>
                        <p className="text-sm">{t("no_used_vouchers_desc")}</p>
                    </div>
                ) : (
